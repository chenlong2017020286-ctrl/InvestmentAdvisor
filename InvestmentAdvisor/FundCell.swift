...
class FundCell: UITableViewCell {
    ...
    private func setupViews() {
        ...
        contentView.addSubview(navLabel)
        ...
        NSLayoutConstraint.activate([
            ...
            navLabel.topAnchor.constraint(equalTo: changeLabel.bottomAnchor, constant: 2),
            navLabel.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -16),
            navLabel.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -8),
        ])
    }
    
    func configure(with fund: Fund) {
        nameLabel.text = fund.name
        codeLabel.text = fund.code
        priceLabel.text = String(format: "%.4f", fund.price)
        navLabel.text = "净值: \(String(format: "%.4f", fund.nav))"
        
        if fund.changePercent >= 0 {
            priceLabel.textColor = .red
            changeLabel.textColor = .red
            changeLabel.text = String(format: "+%.2f%%", fund.changePercent)
        } else {
